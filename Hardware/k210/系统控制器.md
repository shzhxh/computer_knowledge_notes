...
| 8     | clk_sel0      | 时钟选择控制器0。可写，其中aclk_sel设置aclk为时钟源；其中aclk_divider_sel占用2位，控制aclk的分频值；其中apb0~2_clk_sel分别占用3位，分别控制apb0~2的分频值；其中spi3_clk_sel设置spi3为时钟源；其中timer0~2_clk_sel分别设置timer0~2为时钟源；其它为保留位。 |
